...
func parseFile(file FilePathStruct, client *goftp.Client, ftpServerPath string) {
	...
	if strings.Contains(file.file.Name(), "bcms_sp") {
		...
		if strings.Contains(file.file.Name(), "_d") {
			return
		}
		...
		r.DateStamp = strings.TrimSpace(lines[i+2][65:78])
		r.TimeStamp = convertDatetime(r.DateStamp, lines[i+2][52:60])
		r.Number, _ = strconv.Atoi(strings.TrimSpace(lines[i+3][13:44]))
		r.Name = strings.TrimSpace(lines[i+4][13:45])
		r.ServiceLevel, _ = strconv.Atoi(strings.TrimSpace(lines[i+4][74:78]))
		r.FileName = file.file.Name()
		r.SwitchName = strings.TrimSpace(lines[i+2][13:44])
		r.ReportType = "Split"
		...
		r.Number, _ = strconv.Atoi(strings.TrimSpace(lines[i+3][13:44]))
		r.Name = strings.TrimSpace(lines[i+4][13:45])
		r.Trunks, _ = strconv.Atoi(strings.TrimSpace(lines[i+4][74:78]))
		r.FileName = file.file.Name()
		r.SwitchName = strings.TrimSpace(lines[i+2][13:44])
		r.ReportType = "Trunk"
		...
		updateTables(r)
	} else if strings.Contains(file.file.Name(), "bcms_ag") {
		if strings.Contains(file.file.Name(), "day") {
			return
		}
		if strings.Contains(file.file.Name(), "_d") {
		...
		updateTables(r)
	} else if strings.Contains(file.file.Name(), "bcms_vdn") {
		if strings.Contains(file.file.Name(), "day") {
			return
		}
		if strings.Contains(file.file.Name(), "_d") {
